...
class Pipeline implements Serializable {
    ...
    void run(Map config = [:]) {
        ...
        script.stage('SonarQube Analysis') {
            MavenUtils.sonarAnalysis(script, 'spring-minikube/student-service', 'student-service', config.SONARQUBE_ENV ?: 'sonar-java', script.credentials(config.SONARQ_CRED ?: 'anitha-sonar'))
            MavenUtils.sonarAnalysis(script, 'spring-minikube/rating-service', 'rating-service', config.SONARQUBE_ENV ?: 'sonar-java', script.credentials(config.SONARQ_CRED ?: 'anitha-sonar'))
        }
